...
Related: the pipeline **performance** profiler lives at [`../../tests/profile_pipeline.py`](../../tests/profile_pipeline.py); the family self-registry lives on [`ModelParticipant`](../../src/interlens/participant/participants/model_participant.py) (`MODEL_TYPES` + `for_model_type`), and model loading / chat-flag derivation in [`../../src/interlens/loading/load.py`](../../src/interlens/loading/load.py).
