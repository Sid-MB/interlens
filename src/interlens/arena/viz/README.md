...
## Auction episodes: a different page, not a configured one

`GameGeometry` is `GameSpec`-shaped — it materializes the whole `|D| x n` utility matrix of an enumerable deal space — and an auction has neither an enumerable deal space nor a bounded bid set. So `GameGeometry.from_instance` returns `None` on an auction payload and every negotiation panel degrades away; what replaces them is a sibling geometry plus a sibling page. An auction payload sets `game` to `None` explicitly and carries `auction` instead; the two keys are mutually exclusive by construction, and `render_episode_html` dispatches on them so a caller never needs to know which kind of episode it holds.

Four panels, none of which has a negotiation analogue:

1. **The staged bid ladder** — price up, bidding round across, every stage of the episode laid out left to right on one shared price scale so the path reads as a single figure. One polyline per seat through the highest price it committed in each round; on a clock format that is the clock price its `stay`/`claim` happened at, since a clock move carries no amount of its own. Individual lot bids under a multi-lot format are drawn as marks rather than folded into the line (a line through ten simultaneous lot bids would trace a number nobody bid), filled where the bid took the standing high. An irrevocable `exit` is a cross; a Dutch `wait` is **not** — the seat is still in the stage. Each seat's own top realized valuation for the stage is a faint reference tick, and any stage where the outcome rule found an agreement in force is shaded, with the onset stage and every defection labelled on the stage axis.
2. **The per-lot allocation strip** — one bar per lot per stage, each seat's private valuation a tick (the winner's filled), the clearing price the τ-line. A direct adaptation of `page._issue_bars_svg`'s scale-and-tick construction, which is the highest-leverage reuse in the stack. A lot that went unsold carries no τ-line, because an absent clearing price is not a price of zero.
3. **The settlement panel** — winner, payment and per-seat surplus per stage plus the episode total, with every revenue figure beside the benchmark revenue it is measured against. `chrome.stat()` is reused; none of the negotiation fields are, since an auction has no deal, no threshold and no Nash product.
4. **The message graph** — a directed ring over the seats, edge width the DM count, with a stage scrubber that *filters* rather than re-renders (so the graph still reads with scripting off), and the per-dyad counts beside it with a lexical coordination-talk screen's hit count. Per-dyad mutual information with a permutation p-value is deliberately **not** here: it is a cell-level statistic, and over one episode's stages it would be noise wearing a p-value.

### Per-turn counterfactuals

Every committed turn carries what the **rational** rule (an information-conditional Bayesian best response seeing only the acting seat's own information) and the **oracle** rule (the identical best response with every seat's realized valuations attached) would have played *at that same state*. Both are arithmetic given the spec, so both are present on every turn of **every** arm, `all_llm` included — which is what makes an LLM's move comparable to the rules rather than only to other LLMs.

The state is not inferred from the turn log. `auction_trace` replays the episode through the scenario's own state machine and reads `AuctionScenario.state_block(state, seat)` at each turn *before* it is applied, via `replay.apply_prefix`'s `on_request` hook — the pre-move state is the only one at which "what would another rule have played here" means anything, and an `on_turn` hook would score every rule against the position its own turn created. This also fixes stage assignment: under a multi-lot format a stage runs several bidding rounds, so a seat moves many times inside one stage and any "a seat's repeat appearance means a new stage" heuristic reports as many stages as there are rounds.

The self-check with a known answer: on an arm where every seat *is* one of the two rules, that rule's counterfactual must reproduce the played move on every committed turn. It does, which is what licenses reading the same numbers on an all-LLM arm where there is nothing to check them against.

### Non-finite numbers

An auction outcome legitimately carries `NaN`: a stage with no losing bid has no suppression denominator, and an on-path multi-lot benchmark has no counterfactual revenue. Both are "no measurement here" and both are hostile to a page in two separate ways — `json.dumps` emits the bare token `NaN`, which `JSON.parse` rejects and which therefore disables *every* script on the page, and a formatter that prints it renders an absent measurement as the word `nan` beside real numbers. `auction_geometry.json_safe` scrubs the payload at the boundary and `chrome._num` renders non-finite values as an em dash, so both failure modes are closed and the claim the page makes matches the one the analysis makes: absent, not zero.

### The auction index

`AUCTION_INDEX_COLUMNS` replaces the negotiation column set rather than extending it, because half of those columns (deal / distance to NBS / USW / worst-off / total regret) have no auction analogue and would be an em-dash field the length of the table. Difficulty, its parameter tags, and the collusion-onset stage are sortable; a **censored** onset sorts to the far end rather than as stage 0, since a column of censored rows must not look like the earliest onsets in the campaign. `auction_geometry.index_row` is the single owner of what those columns mean, so a campaign hub listing a thousand episodes from their stored records and a rendered run's own index cannot disagree cell for cell.

## Colour

Auction pages need **five** seat identities on one chart, which is two more than the cap below. The cap is a colour-separation constraint, so it is met the way the rest of the stylesheet meets it: colour is never the only encoding. Each seat carries a distinct dash pattern as well as a distinct hue, every mark carries a `<title>` naming its seat, and the legend prints the five names beside their swatches — a reader who cannot separate two lines by colour can separate them by stroke or by hovering, and nothing on the page depends on the distinction being made by colour alone.

Categorical identity is otherwise capped at **three** slots, which is the binding all-pairs constraint of the colour formula for a scatter (validated in both modes with `dataviz/scripts/validate_palette.js --pairs all`: worst all-pairs CVD ΔE 9.2 light / 9.4 dark, normal-vision 24.0 / 20.9). Slot 1 is what the model did (or the left episode), slot 2 the oracle's recommendation (or the right episode), slot 3 the normative solution points. Shape and direct labels distinguish the references within that family: NBS, KS, and EGAL are green stars; UTIL and MNW are violet triangles; per-party ideals are green diamonds. The violet is a redundant reference-point accent, not a fourth trajectory identity—the triangle and direct `UTIL` / `MNW` labels carry the distinction without colour. Deals themselves are chart chrome rather than a series. Slot 3 falls below 3:1 on the light surface, so the relief rule applies — hence the always-visible labels and the numeric table view that ships with every chart.
...
|---|---|
| `geometry.py` | the exact plottable geometry of one negotiation instance: frontier, solutions, party ideals, 2-D embedding |
| `auction_geometry.py` | the `AuctionSpec`-shaped sibling of `geometry.py`, plus `auction_trace` — the replay-derived per-turn state, the per-turn counterfactuals, the ladder series, and `index_row` (the single owner of the auction index's columns) |
| `auction_page.py` | the four auction panels: staged bid ladder, per-lot allocation strip, settlement panel, message graph — plus the per-turn counterfactual table and the auction side panel |
