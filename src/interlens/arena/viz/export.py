...
def render_episode(run: str | Path, episode_path: str | Path, *, reconstruct: bool = True) -> str:
    """The interactive HTML for one episode of a run, as a string."""
    return render_episode_html(RunDir(run).payload(episode_path, reconstruct=reconstruct))


def render_compare(left_run: str | Path, right_run: str | Path, *, index: int = 0,
                   pair_fields: tuple[str, ...] = DEFAULT_PAIR_KEY, reconstruct: bool = True) -> str:
...
def export_episode(run: str | Path, episode_path: str | Path, out_dir: str | Path, *,
                   reconstruct: bool = True) -> Path:
    """Write one episode's page into ``out_dir`` as ``<episode_id>.html`` and return its path."""
    payload = RunDir(run).payload(episode_path, reconstruct=reconstruct)
...
def export_run(run: str | Path, out_dir: str | Path, *, limit: int | None = None,
    ...
    campaign cell with hundreds of episodes."""
    run_dir = RunDir(run)
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
...
def export_comparison(left_run: str | Path, right_run: str | Path, out_dir: str | Path, *,
                      ...
                      reconstruct: bool = True, select: str = "first") -> dict:
    ...
    ``"largest-effect"`` or ``"deal-flip"`` rather than the arbitrary first few.
